...
static int create_token_refresh_timers_from_cache(struct idp_auth_ctx *auth_ctx) {
    const char *attrs[] = {SYSDB_NAME, SYSDB_UUID, SYSDB_ACCESS_TOKEN, SYSDB_REFRESH_TOKEN, NULL};
    ...
    /* TODO: domain stuff, pam_data */
    pd->domain = auth_ctx->be_ctx->domain;
    ...
    if (ret != EOK) {
        DEBUG(SSSDBG_OP_FAILURE, "Failed searching for cached users.\n");
        goto done;
    }
    ...
    for (int i = 0; i < msgs_count; i++) {
        ...
        DEBUG(SSSDBG_TRACE_ALL, "@@ Found user with user uuid: [%s]\n", user_uuid);
                           ...
                           struct dp_method *dp_methods)
{
    ...
    if (dp_opt_get_bool(init_ctx->opts, IDP_AUTO_REFRESH)) {
        ...
        if (auth_ctx->token_refresh_table == NULL) {
            DEBUG(SSSDBG_CRIT_FAILURE, "Failed to create hash table for token refreshes.\n");
            ret = ENOMEM;
            goto done;
        }
        ...
        if (ret != EOK) {
            DEBUG(SSSDBG_OP_FAILURE, "Failed to schedul token refreshes from cache.\n");
            goto done;
        }
    }
